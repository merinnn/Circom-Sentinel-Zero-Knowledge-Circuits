  ...
  if (responseBytes[0] === 0x00 && responseBytes.length >= 2) {
    ...
    if (responseBytes[1] === 0x01) {
      try {
        ...
        const expectedPallet = proofType ? proofConfigurations?.[proofType]?.pallet : undefined;
        const isVerifier = !!section && (section === expectedPallet || getVerifierPallets().has(section));
        return {
          success: false,
          type: 'dispatch_error',
          code,
